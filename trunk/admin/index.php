...
<?php
		foreach ($strObjectArray as $strObject=>$blnValue) {
			printf('<h1>%s</h1><p class="create"><a href="%s/%s_list.php">%s</a> &nbsp;|&nbsp; <a href="%s/%s_edit.php">%s</a></p>',
				$strObject, __VIRTUAL_DIRECTORY__ . __ADMIN__, $strObject, QApplication::Translate('View List'),
				__VIRTUAL_DIRECTORY__ . __ADMIN__, $strObject, QApplication::Translate('Create New'));
		}
?>
		<hr />
		<h1>Panel Drafts &ldquo;Dashboard&rdquo;</h1>
	</div>
