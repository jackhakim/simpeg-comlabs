<?php
	 ...
	 * listing a collection of DataDiklat objects.  It includes
	 ...
	 *
	 */
	class DataDiklatDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataDiklat datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataDiklat object, or it can be any QQNode extending from QQN::DataDiklat().
		 *
		 ...
		 * sorting using two fields from the DataDiklat object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataDiklat
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataDiklat::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataDiklat, given the clauses above
			$this->DataSource = DataDiklat::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataDiklat-based QQNode.
		 * It will also verify that it is a proper DataDiklat-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_diklat') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_diklat".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataDiklat()->Nip;
				case 'NipObject': return QQN::DataDiklat()->NipObject;
				case 'NamaPelatihan': return QQN::DataDiklat()->NamaPelatihan;
				case 'JenisPelatihan': return QQN::DataDiklat()->JenisPelatihan;
				case 'TglMulai': return QQN::DataDiklat()->TglMulai;
				case 'TglSelesai': return QQN::DataDiklat()->TglSelesai;
				case 'StatusLulus': return QQN::DataDiklat()->StatusLulus;
				case 'Predikat': return QQN::DataDiklat()->Predikat;
				case 'Rangking': return QQN::DataDiklat()->Rangking;
				default: throw new QCallerException('Simple Property not found in DataDiklatDataGrid content: ' . $mixContent);
