<?php
	 ...
	 * listing a collection of DataSkLain objects.  It includes
	 ...
	 *
	 */
	class DataSkLainDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataSkLain datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataSkLain object, or it can be any QQNode extending from QQN::DataSkLain().
		 *
		 ...
		 * sorting using two fields from the DataSkLain object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataSkLain
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataSkLain::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataSkLain, given the clauses above
			$this->DataSource = DataSkLain::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataSkLain-based QQNode.
		 * It will also verify that it is a proper DataSkLain-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_sk_lain') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_sk_lain".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataSkLain()->Nip;
				case 'NipObject': return QQN::DataSkLain()->NipObject;
				case 'NoSk': return QQN::DataSkLain()->NoSk;
				case 'JenisSk': return QQN::DataSkLain()->JenisSk;
				default: throw new QCallerException('Simple Property not found in DataSkLainDataGrid content: ' . $mixContent);
