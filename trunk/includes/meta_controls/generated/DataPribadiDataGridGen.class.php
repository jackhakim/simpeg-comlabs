<?php
	 ...
	 * listing a collection of DataPribadi objects.  It includes
	 ...
	 *
	 */
	class DataPribadiDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataPribadi datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataPribadi object, or it can be any QQNode extending from QQN::DataPribadi().
		 *
		 ...
		 * sorting using two fields from the DataPribadi object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataPribadi
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataPribadi::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataPribadi, given the clauses above
			$this->DataSource = DataPribadi::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataPribadi-based QQNode.
		 * It will also verify that it is a proper DataPribadi-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_pribadi') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_pribadi".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataPribadi()->Nip;
				case 'NipObject': return QQN::DataPribadi()->NipObject;
				case 'TempatLahir': return QQN::DataPribadi()->TempatLahir;
				case 'TglLahir': return QQN::DataPribadi()->TglLahir;
				case 'Agama': return QQN::DataPribadi()->Agama;
				case 'StatusNikah': return QQN::DataPribadi()->StatusNikah;
				case 'JumTanggungan': return QQN::DataPribadi()->JumTanggungan;
				case 'Alamat': return QQN::DataPribadi()->Alamat;
				case 'NamaBapak': return QQN::DataPribadi()->NamaBapak;
				case 'NamaIbu': return QQN::DataPribadi()->NamaIbu;
				case 'JenisKelamin': return QQN::DataPribadi()->JenisKelamin;
				default: throw new QCallerException('Simple Property not found in DataPribadiDataGrid content: ' . $mixContent);
