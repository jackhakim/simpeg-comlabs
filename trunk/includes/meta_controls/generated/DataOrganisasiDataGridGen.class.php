<?php
	 ...
	 * listing a collection of DataOrganisasi objects.  It includes
	 ...
	 *
	 */
	class DataOrganisasiDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataOrganisasi datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataOrganisasi object, or it can be any QQNode extending from QQN::DataOrganisasi().
		 *
		 ...
		 * sorting using two fields from the DataOrganisasi object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataOrganisasi
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataOrganisasi::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataOrganisasi, given the clauses above
			$this->DataSource = DataOrganisasi::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataOrganisasi-based QQNode.
		 * It will also verify that it is a proper DataOrganisasi-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_organisasi') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_organisasi".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataOrganisasi()->Nip;
				case 'NipObject': return QQN::DataOrganisasi()->NipObject;
				case 'NamaOrg': return QQN::DataOrganisasi()->NamaOrg;
				case 'Jenis': return QQN::DataOrganisasi()->Jenis;
				case 'Alamat': return QQN::DataOrganisasi()->Alamat;
				case 'ThnMasuk': return QQN::DataOrganisasi()->ThnMasuk;
				case 'ThnKeluar': return QQN::DataOrganisasi()->ThnKeluar;
				case 'Honor': return QQN::DataOrganisasi()->Honor;
				case 'Jabatan': return QQN::DataOrganisasi()->Jabatan;
				default: throw new QCallerException('Simple Property not found in DataOrganisasiDataGrid content: ' . $mixContent);
