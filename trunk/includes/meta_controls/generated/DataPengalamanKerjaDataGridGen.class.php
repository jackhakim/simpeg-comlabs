<?php
	 ...
	 * listing a collection of DataPengalamanKerja objects.  It includes
	 ...
	 *
	 */
	class DataPengalamanKerjaDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataPengalamanKerja datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataPengalamanKerja object, or it can be any QQNode extending from QQN::DataPengalamanKerja().
		 *
		 ...
		 * sorting using two fields from the DataPengalamanKerja object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataPengalamanKerja
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataPengalamanKerja::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataPengalamanKerja, given the clauses above
			$this->DataSource = DataPengalamanKerja::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataPengalamanKerja-based QQNode.
		 * It will also verify that it is a proper DataPengalamanKerja-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_pengalaman_kerja') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_pengalaman_kerja".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataPengalamanKerja()->Nip;
				case 'NipObject': return QQN::DataPengalamanKerja()->NipObject;
				case 'TempatKerja': return QQN::DataPengalamanKerja()->TempatKerja;
				case 'Alamat': return QQN::DataPengalamanKerja()->Alamat;
				case 'TahunMasuk': return QQN::DataPengalamanKerja()->TahunMasuk;
				case 'TahunKeluar': return QQN::DataPengalamanKerja()->TahunKeluar;
				case 'GajiTerakhir': return QQN::DataPengalamanKerja()->GajiTerakhir;
				case 'JabatanTerakhir': return QQN::DataPengalamanKerja()->JabatanTerakhir;
				default: throw new QCallerException('Simple Property not found in DataPengalamanKerjaDataGrid content: ' . $mixContent);
