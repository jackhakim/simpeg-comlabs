<?php
	 ...
	 * listing a collection of DataPegawai objects.  It includes
	 ...
	 *
	 */
	class DataPegawaiDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataPegawai datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataPegawai object, or it can be any QQNode extending from QQN::DataPegawai().
		 *
		 ...
		 * sorting using two fields from the DataPegawai object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataPegawai
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'strNip=<?=urlencode($_ITEM->Nip)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->Nip)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Nip, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataPegawai::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataPegawai, given the clauses above
			$this->DataSource = DataPegawai::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataPegawai-based QQNode.
		 * It will also verify that it is a proper DataPegawai-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_pegawai') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_pegawai".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataPegawai()->Nip;
				case 'Nama': return QQN::DataPegawai()->Nama;
				case 'TglAngkatCapeg': return QQN::DataPegawai()->TglAngkatCapeg;
				case 'TglAngkatPeg': return QQN::DataPegawai()->TglAngkatPeg;
				case 'TglMenjabat': return QQN::DataPegawai()->TglMenjabat;
				case 'Status': return QQN::DataPegawai()->Status;
				case 'Golongan': return QQN::DataPegawai()->Golongan;
				case 'DataAnak': return QQN::DataPegawai()->DataAnak;

				case 'DataDevMandiri': return QQN::DataPegawai()->DataDevMandiri;

				case 'DataDiklat': return QQN::DataPegawai()->DataDiklat;

				case 'DataOrganisasi': return QQN::DataPegawai()->DataOrganisasi;

				case 'DataPasangan': return QQN::DataPegawai()->DataPasangan;

				case 'DataPendidikan': return QQN::DataPegawai()->DataPendidikan;

				case 'DataPengalaman': return QQN::DataPegawai()->DataPengalaman;

				case 'DataPengalamanKerja': return QQN::DataPegawai()->DataPengalamanKerja;

				case 'DataPenghargaanEks': return QQN::DataPegawai()->DataPenghargaanEks;

				case 'DataPenghargaanIn': return QQN::DataPegawai()->DataPenghargaanIn;

				case 'DataPribadi': return QQN::DataPegawai()->DataPribadi;

				case 'DataRiwayatKerja': return QQN::DataPegawai()->DataRiwayatKerja;

				case 'DataSanksi': return QQN::DataPegawai()->DataSanksi;

				case 'DataSkLain': return QQN::DataPegawai()->DataSkLain;

				default: throw new QCallerException('Simple Property not found in DataPegawaiDataGrid content: ' . $mixContent);
