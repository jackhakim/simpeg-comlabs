<?php
	 ...
	 * listing a collection of DataAnak objects.  It includes
	 ...
	 *
	 */
	class DataAnakDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataAnak datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataAnak object, or it can be any QQNode extending from QQN::DataAnak().
		 *
		 ...
		 * sorting using two fields from the DataAnak object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataAnak
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataAnak::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataAnak, given the clauses above
			$this->DataSource = DataAnak::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataAnak-based QQNode.
		 * It will also verify that it is a proper DataAnak-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_anak') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_anak".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataAnak()->Nip;
				case 'NipObject': return QQN::DataAnak()->NipObject;
				case 'NamaAnak': return QQN::DataAnak()->NamaAnak;
				case 'TempatLahir': return QQN::DataAnak()->TempatLahir;
				case 'TglLahir': return QQN::DataAnak()->TglLahir;
				case 'StatusAnak': return QQN::DataAnak()->StatusAnak;
				case 'Agama': return QQN::DataAnak()->Agama;
				case 'Pendidikan': return QQN::DataAnak()->Pendidikan;
				default: throw new QCallerException('Simple Property not found in DataAnakDataGrid content: ' . $mixContent);
