<?php
	 ...
	 * listing a collection of DataDevMandiri objects.  It includes
	 ...
	 *
	 */
	class DataDevMandiriDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataDevMandiri datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataDevMandiri object, or it can be any QQNode extending from QQN::DataDevMandiri().
		 *
		 ...
		 * sorting using two fields from the DataDevMandiri object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataDevMandiri
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataDevMandiri::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataDevMandiri, given the clauses above
			$this->DataSource = DataDevMandiri::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataDevMandiri-based QQNode.
		 * It will also verify that it is a proper DataDevMandiri-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_dev_mandiri') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_dev_mandiri".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataDevMandiri()->Nip;
				case 'NipObject': return QQN::DataDevMandiri()->NipObject;
				case 'NamaKursus': return QQN::DataDevMandiri()->NamaKursus;
				case 'JenisKursus': return QQN::DataDevMandiri()->JenisKursus;
				case 'Penyelenggara': return QQN::DataDevMandiri()->Penyelenggara;
				case 'Alamat': return QQN::DataDevMandiri()->Alamat;
				case 'TglMulai': return QQN::DataDevMandiri()->TglMulai;
				case 'TglSelesai': return QQN::DataDevMandiri()->TglSelesai;
				case 'NoSertifikat': return QQN::DataDevMandiri()->NoSertifikat;
				case 'Predikat': return QQN::DataDevMandiri()->Predikat;
				default: throw new QCallerException('Simple Property not found in DataDevMandiriDataGrid content: ' . $mixContent);
