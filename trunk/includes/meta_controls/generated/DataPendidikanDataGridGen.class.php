<?php
	 ...
	 * listing a collection of DataPendidikan objects.  It includes
	 ...
	 *
	 */
	class DataPendidikanDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataPendidikan datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataPendidikan object, or it can be any QQNode extending from QQN::DataPendidikan().
		 *
		 ...
		 * sorting using two fields from the DataPendidikan object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataPendidikan
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataPendidikan::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataPendidikan, given the clauses above
			$this->DataSource = DataPendidikan::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataPendidikan-based QQNode.
		 * It will also verify that it is a proper DataPendidikan-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_pendidikan') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_pendidikan".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataPendidikan()->Nip;
				case 'NipObject': return QQN::DataPendidikan()->NipObject;
				case 'Jenjang': return QQN::DataPendidikan()->Jenjang;
				case 'NamaTempat': return QQN::DataPendidikan()->NamaTempat;
				case 'TahunMasuk': return QQN::DataPendidikan()->TahunMasuk;
				case 'TahunLulus': return QQN::DataPendidikan()->TahunLulus;
				case 'Gelar': return QQN::DataPendidikan()->Gelar;
				case 'NoIjasah': return QQN::DataPendidikan()->NoIjasah;
				default: throw new QCallerException('Simple Property not found in DataPendidikanDataGrid content: ' . $mixContent);
