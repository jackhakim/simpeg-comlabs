<?php
	 ...
	 * listing a collection of DataSanksi objects.  It includes
	 ...
	 *
	 */
	class DataSanksiDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataSanksi datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataSanksi object, or it can be any QQNode extending from QQN::DataSanksi().
		 *
		 ...
		 * sorting using two fields from the DataSanksi object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataSanksi
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataSanksi::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataSanksi, given the clauses above
			$this->DataSource = DataSanksi::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataSanksi-based QQNode.
		 * It will also verify that it is a proper DataSanksi-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_sanksi') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_sanksi".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataSanksi()->Nip;
				case 'NipObject': return QQN::DataSanksi()->NipObject;
				case 'JenisSanksi': return QQN::DataSanksi()->JenisSanksi;
				case 'NoSk': return QQN::DataSanksi()->NoSk;
				default: throw new QCallerException('Simple Property not found in DataSanksiDataGrid content: ' . $mixContent);
