<?php
	 ...
	 * listing a collection of DataPengalaman objects.  It includes
	 ...
	 *
	 */
	class DataPengalamanDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this DataPengalaman datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DataPengalaman object, or it can be any QQNode extending from QQN::DataPengalaman().
		 *
		 ...
		 * sorting using two fields from the DataPengalaman object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DataPengalaman
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DataPengalaman::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from DataPengalaman, given the clauses above
			$this->DataSource = DataPengalaman::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DataPengalaman-based QQNode.
		 * It will also verify that it is a proper DataPengalaman-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'data_pengalaman') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "data_pengalaman".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Nip': return QQN::DataPengalaman()->Nip;
				case 'NipObject': return QQN::DataPengalaman()->NipObject;
				case 'Pengalaman': return QQN::DataPengalaman()->Pengalaman;
				case 'Tanggal': return QQN::DataPengalaman()->Tanggal;
				default: throw new QCallerException('Simple Property not found in DataPengalamanDataGrid content: ' . $mixContent);
